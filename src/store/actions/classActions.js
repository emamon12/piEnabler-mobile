...
export const addResponse = fbase => (dispatch, getState, { getFirestore }) => {
  ...
  if (
    resp.userAnswer === 'answer1' ||
    resp.userAnswer === 'answer2' ||
    resp.userAnswer === 'answer3' ||
    resp.userAnswer === 'answer4'
  ) {
    fireStore
      .collection('responses')
      .doc(
        `${sessionid}${studentId}${session.currentSliceId}${session.numPolls}`
      )
      .set({
...
export const ChangeMessage = classs => (
  dispatch,
  getState,
  { getFirestore }
) => {
...
export const RemoveSession = classs => (
  dispatch,
  getState,
  { getFirestore }
) => {
